...
{ 
	...
	do
		...
		::atomic{ (Pos==1 && Dir==-1 && (B2==allume || BD2==allume || BM1==allume)) -> DirPrec=Dir ; Dir=1} progress2 : skip;
		::atomic{ (Pos==1 && Dir==1 && (B0==allume || BM0==allume || BD1==allume)) -> DirPrec=Dir ; Dir=-1} progress3 : skip;
		...
		::atomic {(Pos==1 || Pos==2) -> B0=allume;} progress4 : skip;
		/*Appel depuis cabine de l'étage 1  */
		::atomic {(Pos==0 || Pos==2) -> B1=allume;} progress5 : skip;
		/*Appel depuis cabine de l'étage 0  */
		::atomic {(Pos==0 || Pos==1) -> B2=allume;} progress6 : skip;
		
		/*Réception de l'état des boutons des étages */
		/*Demande pour monter de l'étage 0 */
		::atomic{EC? 1,0 ->BM0=allume;}
		/*Demande pour monter de l'étage 1 */
		::atomic{EC? 1,1 ->BM1=allume;} 
		/*Demande pour descendre de l'étage 1 */
		::atomic{EC? -1,1 ->BD1=allume;} 
		/*Demande pour descendre de l'étage 2 */
		::atomic{EC? -1,2 ->BD2=allume;}
...
ltl p1{[]( (Cabine:Pos == 0 && Cabine:DirPrec == -1) -> (Cabine:Pos == 0 U Cabine:Dir == 1))}; 
